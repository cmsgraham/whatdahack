...
def _slugify(value):
    slug = re.sub(r"[^a-z0-9]+", "-", (value or "challenge").lower()).strip("-")
    return slug or "challenge"


def _build_local_bundle(chal):
    """Build a self-contained 'run it locally' bundle (docker-compose + README).

    Returns (bytes, filename). The bundle references the same public Docker Hub
    image the hosted instance uses, so players can practice offline. It is a
    throwaway local copy: flags are still submitted on the platform.
    """
    slug = _slugify(chal.name)
    image = chal.instance_image
    mode = (chal.connect_mode or "ssh").lower()
    console = mode in ("console", "both")
    ssh = mode in ("ssh", "both")
    ...
    if console:
        ports.append('      - "7681:7681"')

    env = ['      PLAYER_PASSWORD: "%s"' % password]
    if console:
        env.append('      CONSOLE_ENABLED: "1"')

    limits = []
    if chal.memory_mb:
        limits.append("    mem_limit: %dm" % int(chal.memory_mb))
    if chal.cpus:
        try:
            limits.append("    cpus: %s" % float(chal.cpus))
        except (TypeError, ValueError):
            pass
    if chal.pids:
        limits.append("    pids_limit: %d" % int(chal.pids))

    compose = "\n".join(
        [
            "# Local practice copy of: %s" % chal.name,
            "# Generated by whatdahack. This is a throwaway environment for",
            "# practicing offline; submit your flag on the platform.",
            "services:",
            "  instance:",
            "    image: %s" % image,
            "    container_name: wdh-%s" % slug,
            "    ports:",
            *ports,
            "    environment:",
            *env,
    ...
    lines = [
        "# %s — Local Practice Instance" % chal.name,
        "",
        "Run this challenge's environment on your own machine.",
        "",
        "## Requirements",
        "",
    ...
    if console:
        lines += [
            "## Browser console",
            "",
            "Open <http://localhost:7681> and log in as `player` with the",
            "password `%s`." % password,
            "",
        ]
    lines += [
        ...
        "## Notes",
        "",
        "- This local copy is **not** sandboxed or network-restricted the way the",
        "  hosted instance is. Run it at your own discretion.",
        "- It is for **practice only** — submit your flag on the platform to score.",
        "",
    ]
    readme = "\n".join(lines)
...
def _register_player_routes(app):
    ...
    @player_bp.route(
        "/plugins/instance_challenges/instances/export", methods=["GET"]
    )
    @authed_only
    def export_local():
        try:
        ...
        buf, filename = _build_local_bundle(chal)
        return send_file(
            buf,
